...
let test_name name = "union-find-lattice."^name
...
module Value (*: Parameters.POLYMORPHIC_VALUE*) = struct
...
module PA = Polymorphic.Persistent.GenericRelationalValued(Elt)(Relation)(Value)
...
let rec remove_duplicate cast set = function
  | [] -> []
  | t::q when Hashtbl.mem set (cast t) -> remove_duplicate cast set q
  | t::q ->
      let () = Hashtbl.add set (cast t) () in
      t :: remove_duplicate cast set q

(** Transform a list of list into a partition (list of list with no duplicates) *)
let rec make_partition cast set = function
  | [] -> []
  | t::q -> let t = remove_duplicate cast set t in
            let q = make_partition cast set q in
            if t = [] then q else t::q

(** Renumber random numbers into a contiguous 0..max_number segment *)
let renumber seen max_number (i,j,k) =
  match Hashtbl.find_opt seen i with
  | Some n -> (n,j,k)
  | None ->
      let n = !max_number in
      Hashtbl.add seen i n;
      incr max_number;
      (n,j,k)
let renumber_partition p =
  let seen = Hashtbl.create 100 in
  let max_number = ref 0 in
  let list = List.map (List.map (renumber seen max_number)) p in
  list, !max_number

let gen_partition =
  QCheck.map
    ~rev:(fun x -> x)
    (make_partition (fun (i,_,_) -> i) (Hashtbl.create 100))
    QCheck.(list_small (list_small (triple int_pos (option int) (option int))))
...
let pa_model_from_gen gen size =
  let rec create_class hd uf = function
    | [] -> uf
    | (i, rel, _) :: q ->
        let rel = match rel with None -> Relation.Equal | Some x -> relation_of_int x in
        match PA.add_relation uf hd (IntTerm i) rel with
        | Ok uf -> create_class hd uf q
        | Error _rel -> failwith "Duplicate union"
  in
  List.fold_left (fun uf classe -> match classe with
    ...
    | (i, im, iM)::q ->
        let hd = Elt.IntTerm i in
        let uf = PA.add_value uf hd (Value.interval (to_z im) (to_z iM)) in
        create_class hd uf q
  ) (PA.make size) gen
...
let check_models_agree pa_model fun_model imp_model =
  let check_class hd t =
      ...
      let pa_rel = Option.get (PA.check_related pa_model (payload t) (payload hd)) in
      if Relation.equal imp_rel fun_rel && Relation.equal imp_rel pa_rel then true
      else (
        QCheck2.Test.fail_reportf
          "Different relations between %a and %a :\n %a (imperative), %a (functional), %a parray@."
          Elt.pretty (payload t) Elt.pretty (payload hd)
          Relation.pretty imp_rel Relation.pretty fun_rel Relation.pretty pa_rel
      )
  in
...
let test_agree = QCheck.Test.make ~count:1000 ~name:(test_name "functional&imperative.build") gen_partition
  (fun part ->
    let part, size = renumber_partition part in
    check_models_agree (pa_model_from_gen part size) (fun_model_from_gen part) (imp_model_from_gen part))

let test_invariants = QCheck.Test.make ~count:1000 ~name:(test_name "functional.build-invariants") gen_partition
  (fun part -> check_invariants (fun_model_from_gen part))

(* let test_union = QCheck.Test.make ~count:1000 ~name:(test_name "functional&imperative.union")
...
let test_join = QCheck.Test.make ~count:1000 ~name:(test_name "functional.join-random")
...
let test_join_with_previous = QCheck.Test.make ~count:1000 ~name:(test_name "functional.join-with-previous")
  QCheck.(pair gen_partition (list_small int))
  (fun (part, unions) ->
...
let test_join_two_different = QCheck.Test.make ~count:1000 ~name:(test_name "functional.join-common-ancestor")
  QCheck.(triple gen_partition (list_small int) (list_small int))
...
let tests = [
  ...
  (* test_union; *)
  test_join;
  test_join_with_previous;
  test_join_two_different;
]
