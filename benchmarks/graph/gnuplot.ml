...
module Color = struct
  ...
  let lighten_component n = (n + 2*255) / 3

  let lighten x =
    let r,g,b = to_rgb x in
    `Rgb(lighten_component r, lighten_component g, lighten_component b)
end
...
let format_style = function
  | `Solid -> " fs solid 0.3"
  | `TranspPattern n -> sprintf " fs transparent pattern %d" n
...
module Output = struct
  ...
  let to_cmd t : Command.t =
    let all_args =
      ...
      (t.size |> format_arg (fun (x,y) -> sprintf " size %s,%s" x y)) ^
    ...
    let output =
      ...
      | `Eps_latex s ->
        sprintf "set term epslatex%s\nset output '%s'" all_args s
      | `Latex_cairo s ->
        sprintf "set term cairolatex%s\nset output '%s'" all_args s
    in
    { Command.
      command = output;
...
module Labels = struct
  type t = {
    x : string option;
    y : string option;
    yoffset: string option;
  }

  let create ?x ?y ?yoffset () = { x; y; yoffset }

  let to_cmd t =
    let cmd =
      [ format_label "xlabel" t.x, "set xlabel"
      ; format_label "ylabel" t.y ^ format_arg (sprintf " offset %s") t.yoffset, "set ylabel"
...
type data =
  ...
  | Data_XY_Err_Y of (float * float * float) list
...
module Series = struct
  ...
  let create ?title ?color ?weight ?fill ?point_type ?dash_type kind data =
    let kind_text =
      match kind with
      | Lines -> "lines"
      | Points -> "points"
      ...
      | Errorlines -> "yerrorlines"
      | Errorshade -> "filledcurves"
    in
    let cmd =
      String.concat "" [
        (match data with
         ...
         | Data_XY_Err_Y _ -> " '-' using 1:2:3 with " ^ kind_text)
      ; format_title title
      ; format_num_arg "lw" weight
      ; format_color "lc" color
      ; format_num_arg "pt" point_type
      ; format_arg (sprintf "dt %s") dash_type
      ; format_fill fill ]
    in
    { cmd; data; }
  ...
  let lines_xy ?title ?color ?weight ?dash_type data =
    create ?title ?color ?weight ?dash_type Lines (Data_XY data)
  ...
  let linespoints_xy ?title ?color ?weight ?dash_type data =
    create ?title ?color ?weight ?dash_type Linespoints (Data_XY data)
  ...
  let yerror_bars ?title ?color ?weight ?point_type ?dash_type data =
    create ?title ?color ?weight ?point_type ?dash_type Errorlines (Data_XY_Err_Y data)

  let yerror_shade ?title ?color ?fill ?weight ?point_type ?dash_type data =
    let xy_data = List.map (fun (x,y,_) -> (x,y)) data in
    let err_data = List.map (fun (x,y,d) -> (x,y -. d, y+.d)) data in
    let color2 = Option.map Color.lighten color in
    [ create ?color:color2 ?fill Errorshade (Data_XY_Err_Y err_data);
      create ?title ?color ?weight ?point_type ?dash_type Linespoints (Data_XY xy_data) ]
end
...
let send_data t data =
  ...
  | Func _ -> ()
  | Data_XY_Err_Y data ->
    List.iter (fun (x,y,delta) ->
      send_cmd t (format_num x ^ " " ^ format_num y ^ " " ^ format_num delta)) data;
      send_cmd t "e"
