...
val standard_deviation_percent: ('a, 'kind) t -> float
(** The {!standard_deviation} as a percentage of the {!average}.
    [100.] means the standard deviation is equal to the average. *)
...
val stats_of_sum: ('a, 'k1) t -> ('a, 'k2) t -> ('a, compact) t
(** [stats_of_sum (mk [1;2;3]) (mk [4;5;6])] is the same as [mk [1+4;2+5;3+6]].
    {b Warning:} when the collections are not sorted, the {!min} and {!max} will
    be incorrect. *)
...
val float_prefixes : string list
(** Standard SI unit prefix for values < 1: ["m"; "µ"; "n"; "p"; "f"; "a"; "z"; "y"; "r"; "q"] *)

val pp_int_with_unit:
  ?justify:bool ->
...
(** [pp_int_with_unit () fmt nb] prints the number nb with at most three digits using the
...
val pp_float_with_unit:
  ?justify:bool ->
  ?unit_prefixes:string list ->
  ?float_prefixes: string list ->
  ?separator:string ->
  ?base:int ->
  unit ->
  Format.formatter -> float -> unit
(** Same as {!pp_int_with_unit}, but with floats. [float_prefixes] are used for
